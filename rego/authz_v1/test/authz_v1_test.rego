...
test_has_action_picks_up_INLINE_action {
	has_action[["polid", "statementid"]] with data.policies.policies.polid.statements.statementid.actions as ["x"]
		 with input.action as "x"
}

test_has_action_ignores_other_inline_action {
	not has_action[["polid", "statementid"]] with data.policies.policies.polid.statements.statementid.actions as ["y"]
		 with input.action as "x"
}

test_has_action_picks_up_ROLE_action {
	has_action[["polid", "statementid"]] with data.policies.policies.polid.statements.statementid.role as "editor"
		 with data.policies.roles.editor.actions as ["x"]
		 with input.action as "x"
}

test_has_action_ignores_same_action_from_different_role {
	not has_action[["polid", "statementid"]] with data.policies.policies.polid.statements.statementid.role as "editor"
		 with data.policies.roles.viewer.actions as ["x"]
		 with input.action as "x"
}

test_has_action_ignores_same_inline_action_from_different_statement {
	not has_action[["polid", "statement1"]] with data.policies.policies.polid.statements as {
		{"statement1": {"action": ["y"]}},
		{"statement2": {"action": ["x"]}},
	}
		 with input.action as "x"
...
test_has_resource_picks_up_resource_from_policy_statement_data {
 	has_resource[["polid", "statementid"]] with data.policies.policies as {"polid": {"statements": {"statementid": {"resources": ["y"]}}}}
...
test_has_member_picks_up_member_from_policy_data {
	has_member.polid with data.policies.policies as {"polid": {"members": ["z"]}}
...
test_allow_matches_all_properties_with_INLINE_action_and_effect_allow {
	allow with data.policies.policies.polid as {"members": ["x"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y"], "resources": ["z"]}}}
...
test_allow_matches_all_properties_with_ROLE_action_and_effect_allow {
	allow with data.policies.policies.polid as {"members": ["x"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "role": "editor", "resources": ["z"]}}}
		 with data.policies.roles.editor.actions as ["y"]
		 with input as {"subjects": ["x"], "action": "y", "resource": "z", "organizationId": "1234"}
}

test_deny_matches_all_properties_and_effect_deny {
	deny with data.policies.policies.polid as {"members": ["x"], "organizationId": "1234", "statements": {"statementid": {"effect": "deny", "actions": ["y"], "resources": ["z"]}}}
		 with input as {"subjects": ["x"], "action": "y", "resource": "z", "organizationId": "1234"}
}
...
test_not_authorized_when_only_not_matching_policies_with_effect_allow_are_present {
	not authorized with data.policies.policies.polid as {"members": ["x0"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["z0"]}}}
...
test_authorized_when_one_among_a_group_of_members_is_present {
	authorized with data.policies.policies.polid as {"members": ["x0", "x1"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["z0"]}}}
...
test_authorized_when_not_all_subjects_are_present_as_members {
	authorized with data.policies.policies.polid as {"members": ["x1"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["z0"]}}}
...
test_authorized_when_not_matching_policy_with_effect_deny_is_present {
	authorized with data.policies.policies.polid as {"members": ["x"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y"], "resources": ["z"]}}}
...
test_not_authorized_when_any_matching_policy_with_effect_deny_is_present {
	not authorized with data.policies.policies.polid0 as {"members": ["x"], "organizationId": "1234", "statements": {"statementid0": {"effect": "allow", "actions": ["y"], "resources": ["z"]}}}
		 with data.policies.policies.polid1 as {"members": ["x"], "organizationId": "1234", "statements": {"statementid1": {"effect": "deny", "actions": ["y"], "resources": ["z"]}}}
		 with input as {"subjects": ["x"], "action": "y", "resource": "z", "organizationId": "1234"}
}

test_not_authorized_matches_all_properties_except_organization {
	not authorized with data.policies.policies.polid as {"members": ["x"], "organizationId": "w", "statements": {"statementid": {"effect": "allow", "actions": ["y"], "resources": ["z"]}}}
		 with input as {"subjects": ["x"], "action": "y", "resource": "z", "organizationId": "w1"}
}

test_authorized_when_resource_is_hierarchical_env {
	authorized with data.policies.policies.polid as {"members": ["x1"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["crn://e0"]}}}
...
test_not_authorized_when_resource_is_hierarchical_env {
	not authorized with data.policies.policies.polid as {"members": ["x1"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["crn://e0"]}}}
	     with data.tenants as [ { "id": "o1", "envs": [ { "id": "e1", "crn": "crn://e1", "clusters" : [ { "id": "z0" } ] } ] } ]
		 with input as {"subjects": ["x1"], "action": "y0", "resource": "z0", "organizationId": "1234"}
}

test_authorized_when_resource_is_hierarchical_org {
	authorized with data.policies.policies.polid as {"members": ["x1"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["crn://o0"]}}}
...
test_not_authorized_when_resource_is_hierarchical_org {
	not authorized with data.policies.policies.polid as {"members": ["x1"], "organizationId": "1234", "statements": {"statementid": {"effect": "allow", "actions": ["y0"], "resources": ["crn://o0"]}}}
